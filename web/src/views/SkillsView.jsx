...
// monochrome: categories differ by tone, never by hue
const COLORS = {
  reasoning: 'var(--text-0)', design: 'var(--text-1)', statistics: 'var(--text-0)',
  mathematics: 'var(--text-1)', text: 'var(--text-2)', literature: 'var(--text-1)',
  data: 'var(--text-2)', code: 'var(--text-0)', neural: 'var(--text-1)', utility: 'var(--text-3)',
};
        ...
        <div className="row wrap" style={{ marginBottom: 18 }}>
          ...
          {(skills.categories || []).map((c) => (
            <button
              ...
              onClick={() => setCat(c.id)}
            >
              {c.label} <span className="mono" style={{ fontSize: 10, opacity: 0.6 }}>{c.skills.length}</span>
            </button>
          ))}
        </div>
