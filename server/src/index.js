...
import { probeProviders, snapshot as providerSnapshot, tlsBootstrap } from './web/providers.js';
...
const researcher = new WebResearcher(runtime);
probeProviders({ force: true }).then((st) => {
  const on = st.filter((p) => p.online).map((p) => p.id);
  console.log(`[web] TLS: ${tlsBootstrap.applied ? tlsBootstrap.source : `default (${tlsBootstrap.reason})`}`);
  console.log(`[web] providers online: ${on.length ? on.join(', ') : 'none — running fully offline'}`);
}).catch((e) => console.warn('[web] probe failed', e.message));
setInterval(() => probeProviders({ force: true }).catch(() => {}), 600000).unref();
...
app.get('/api/web/status', async (_req, res) => {
  res.json({
    tls: tlsBootstrap,
...
app.post('/api/web/search', async (req, res) => {
  try {
    const dossier = await researcher.searchOnce(req.body?.query || '', {
      maxResults: Number(req.body?.limit) || 8,
      readDocs: Number(req.body?.read ?? 2),
...
app.post('/api/chat/stream', async (req, res) => {
  ...
  const events = [];
  try {
    for await (const ev of runAgent({ message, mode, settings, runtime, registry, researcher })) {
      if (closed) break;
