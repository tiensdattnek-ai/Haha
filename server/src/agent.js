...
import { renderDossier } from './web/researcher.js';
...
/** Priors the router cannot learn from tag text alone. */
function numericPriors(message) {
  const nums = numbers(message);
  const groups = String(message).split(/\bvs\.?\b|\bversus\b|\bcompared? (?:to|with)\b|;|\n/i)
    .map((p) => numbers(p)).filter((p) => p.length >= 3);
  const bonus = {};
  if (nums.length >= 4 && groups.length >= 2) {
    bonus['stats.ttest'] = 3.2;
    bonus['stats.nonparametric'] = 1.2;
    bonus['stats.bootstrap'] = 0.9;
    bonus['stats.describe'] = 0.8;
  } else if (nums.length >= 5) {
    bonus['stats.describe'] = 2.4;
    bonus['data.timeseries'] = /over time|trend|series|month|day|week/i.test(message) ? 2.2 : 0;
  }
  if (groups.length >= 3) bonus['stats.anova'] = 3.0;
  if (/\bcorrelat|relationship between|scatter/i.test(message) && nums.length >= 6) bonus['stats.correlation'] = 3.0;
  return bonus;
}
...
function classify(message, registry, runtime) {
  ...
  for (const id of Object.keys(numericPriors(message))) {
    if (!merged.has(id)) merged.set(id, { id, heuristic: 0, neural: 0 });
  }
  const priors = numericPriors(message);
  const ranked = [...merged.values()]
    .map((x) => {
      ...
      return {
        ...
        prior: priors[x.id] || 0,
        score: x.heuristic * 0.62 + x.neural * 2.6 + (priors[x.id] || 0),
      };
    })
...
function admissible(skill, message, nums) {
  ...
  const CODEISH = /```|[;{}]|=>|\bfunction\b|\bconst \w+ *=|\bdef \b|\bclass \b|\bimport \b|\breturn\b|\w+\(\)/;
  const guards = {
    'code.run': CODEISH,
    'code.review': CODEISH,
    'code.explain': CODEISH,
    'code.test': CODEISH,
    'code.complexity': CODEISH,
    'code.convert': CODEISH,
    'code.scaffold': /scaffold|boilerplate|starter|new project|project structure|set ?up/i,
    'code.regex': /regex|regular expression|pattern/i,
    'text.compare': /(?=[\s\S]*[a-z]{4})(?:\bvs\.?\b|versus|compare|similar|plagiar|overlap)/i,
  ...
  // a numeric comparison is a statistics job, not a text-similarity job
  if (skill.id === 'text.compare' && nums.length >= 4) return false;
  if (skill.id === 'stats.bayes' && !/bayes|prior|posterior|credible|success|trial|out of/i.test(m)) return false;
  if (skill.id === 'text.summarize' && nums.length >= 5 && String(m).replace(/[\d\s.,;-]/g, '').length < 40) return false;
  return true;
}

/* ------------------------------ the pipeline ------------------------------ */

export async function* runAgent({ message, mode = 'deep', settings = {}, runtime, registry, researcher = null, history = [] }) {
  const t0 = Date.now();
  ...
  if (cmd) {
    if ((cmd.cmd === 'web' || cmd.cmd === 'search') && researcher) {
      const q = cmd.rest || message;
      yield emit('phase', { phase: 'websearch', label: PHASES.websearch });
      const it = researcher.research(q, { maxResults: 8, readDocs: 3 });
      let step = await it.next();
      while (!step.done) { yield emit(step.value.type, step.value); step = await it.next(); }
      const dossier = step.value;
      const md = renderDossier(dossier, { title: `Live search — “${q}”` }) || '_No provider returned results. Check `/api/web/status` for provider availability._';
      for (const piece of chunkStream(md)) yield emit('delta', { text: piece });
      yield emit('done', { skills: ['web.search'], sources: dossier.results.map((r) => r.url), confidence: dossier.results.length ? 0.8 : 0.2, ms: Date.now() - t0 });
      return;
    }
...
- \`/help\` for the full catalogue, \`/model.card\` for my provenance.`;
  ...
  // ---------- live web research ---------------------------------------------
  let dossier = null;
  const wantWeb = settings.web !== false && researcher
    && (deep || /\b(search|latest|current|find|source|paper|library|package|documentation|news|version)\b/i.test(message));
  if (wantWeb) {
    yield emit('phase', { phase: 'websearch', label: PHASES.websearch });
    try {
      const it = researcher.research(message, { maxResults: settings.webResults || 6, readDocs: settings.webDocs ?? 2 });
      let step = await it.next();
      while (!step.done) { yield emit(step.value.type, step.value); step = await it.next(); }
      dossier = step.value;
    } catch (e) {
      yield emit('warning', { message: `live search failed: ${e.message}` });
    }
  }
  ...
  if (dossier && dossier.results?.length) {
    for (const piece of chunkStream(renderDossier(dossier))) yield emit('delta', { text: piece });
  }
  ...
  if (deep) {
    ...
    confidence = Math.max(0.15, Math.min(0.96,
      0.35 + 0.15 * ok + (sources.length ? 0.15 : 0) + (dossier?.docs?.length ? 0.14 : 0)
      + (nums.length > 2 ? 0.1 : 0) - (results.length === 0 ? 0.3 : 0)));
  ...
  yield emit('done', {
    ...
    web: dossier ? { results: dossier.results.length, documents: dossier.docs.length, providers: [...new Set(dossier.results.map((r) => r.source))], ms: dossier.ms } : null,
