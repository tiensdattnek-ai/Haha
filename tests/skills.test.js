...
const FIXTURES = {
  ...
  'data.code': { code: 'const xs=[1,2,3,4]; xs.reduce((a,b)=>a+b,0)/xs.length' },
...
test('registry exposes a coherent catalogue', () => {
  ...
  assert.ok(skills.length >= 55, `expected 55+ skills, got ${skills.length}`);
  const ids = new Set();
...
test('every non-neural instrument executes and returns substantive markdown', async () => {
  ...
  for (const [id, args] of Object.entries(FIXTURES)) {
    try {
      ...
      if (/undefined|NaN\b/.test(r.markdown.replace(/`NaN`/g, ''))) {
        failures.push(`${id}: emitted undefined/NaN`);
      }
    } catch (e) {
