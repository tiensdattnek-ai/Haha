...
import { buildConversations, buildCodeDocs, CONVERSATION_STATS } from './conversation.js';
 ...
 * Returns { text, stats }.
 */
export function buildCorpus({ seed = 7, targetChars = 1_200_000, chatRatio = 0.45, codeRatio = 0.3, dialogueRatio = 0.28 } = {}) {
  ...
  // 4. assistant dialogue (register) and code documentation (syntax)
  const dialogue = buildConversations({ rng, targetChars: Math.round(targetChars * dialogueRatio) });
  const codeDocs = buildCodeDocs({ rng, targetChars: Math.round(targetChars * codeRatio) });
  parts.push(dialogue, codeDocs);
  chars += dialogue.length + codeDocs.length;
  ...
  return {
    ...
    stats: {
      ...
      chatExchanges: (text.match(/<bos>/g) || []).length,
      codeBlocks: codeFences,
      codeChars: codeDocs.length,
      dialogueChars: dialogue.length,
      recipes: CONVERSATION_STATS.recipes,
    },
  };
}
