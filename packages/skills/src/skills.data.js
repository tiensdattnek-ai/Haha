...
${issues.length ? issues.map((i) => `- ${i}`).join('\n') : '- None detected.'}`,
  ...
  {
    id: 'data.code',
    name: 'Sandboxed JS Runtime',
    category: 'data',
    summary: 'Executes JavaScript in an isolated VM with a 3s budget — for quick numeric experiments.',
    tags: ['code', 'run', 'javascript', 'execute', 'script', 'compute'],
    params: [
      { name: 'code', type: 'string', required: true, description: 'JavaScript source; the last expression or console output is returned' },
      { name: 'timeout', type: 'number', default: 3000, description: 'ms budget' },
    ],
    match: (m) => (/```|\b(run this code|execute|javascript|write a (function|script))\b/i.test(m) ? 2 : 0),
    run: ({ code, timeout = 3000 }) => {
      const logs = [];
      const sandbox = {
        console: {
        ...
        Math, JSON, Date, Array, Object, Number, String, Boolean, Map, Set,
        Float64Array, Float32Array, Int32Array, BigInt, RegExp, Promise, isNaN, isFinite, parseFloat, parseInt,
      };
      ...
      let error = null;
      try {
        value = vm.runInContext(String(code), ctx, { timeout: Math.min(10000, Number(timeout) || 3000), displayErrors: true });
      } catch (e) {
      ...
      const fmtVal = (v) => {
        ...
        try { return typeof v === 'object' ? JSON.stringify(v, null, 2).slice(0, 4000) : String(v); } catch { return String(v); }
      };
      return {
        markdown: `## Execution ${error ? '❌ failed' : '✅ ok'} (${ms} ms)

${logs.length ? `**stdout**\n\`\`\`\n${logs.join('\n').slice(0, 4000)}\n\`\`\`\n` : ''}
${error ? `**error**\n\`\`\`\n${error}\n\`\`\`` : `**return value**\n\`\`\`js\n${fmtVal(value)}\n\`\`\``}

> Isolated \`node:vm\` context — no filesystem, no network, no \`require\`, hard time budget. Determinism is your responsibility (seed your RNG).`,
        data: { logs, value: error ? null : fmtVal(value), error, ms },
      };
    },
  },
